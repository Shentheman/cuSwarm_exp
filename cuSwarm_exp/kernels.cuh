...
void launchMainKernel(float3 gh, float2 gp, uint sn, int* leaders, bool* ap, Parameters p, 
...
void getData(uint n, float4* positions, float3* velocities, int* modes, int* nearest_leader, uint* leader_countdown);
void getLaplacian(uint n, int4* laplacian);
void setData(uint n, float4* positions, float3* velocities, int* modes);
void setData(uint n, float4* positions, float3* velocities, int* modes, int* nearest_leader, uint* leader_countdown);
...
__global__ void main_kernel(float4* pos, float3* vel, int* mode, float3 goal_heading, float2 goal_point, curandState* rand_state, bool* ap, float2* flow_pos, float2* flor_dir, bool* occupancy, Parameters p, uint sn);

__device__ void rendezvous(float3 dist3, float2* min_bounds, float2* max_bounds, float2* repel, bool is_ap, Parameters p);

__device__ void flock(int myMode, float3 nVel, int nMode, float3 dist3, float2* repel, float2* align, float2* cohere, bool is_ap, Parameters p);

__device__ void disperse(float3 dist3, float2* repel, float2* cohere, bool is_ap, Parameters p);

__device__ void rendezvousToPoint(float3 dist3, float2* repel, Parameters p);

__device__ void obstacleAvoidance(float4 myPos, float2* avoid, float* dist_to_obstacle, bool* occupancy, Parameters p);

__device__ bool checkOccupancy(float x, float y, bool* occupancy, Parameters p);

__device__ void setColor(uchar4* color, int mode, bool is_ap, uint i, Parameters p);
